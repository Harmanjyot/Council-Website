...
<html lang="en">
          ...
          <div class="tile">
            <div class="tile-body">
              <div class="container box">
                 <h1 align="center">Edit Registrations</h1>
                 <br>
                 <div class="optionList">
                   <form method="post" action="">
                    ...
                    $query = "SELECT * FROM eventList ORDER BY eventType";
                    $result = mysqli_query($conn, $query);
                    if(mysqli_num_rows($result) > 0)  
                            {  
                                while($row = mysqli_fetch_array($result))  
                                { ?>
                                  <option name = "<?php echo $row["SrNo"]; ?>"><?php echo $row["eventName"];?></option>
 ...
 $(document).ready(function(){  
      $("#listEvent").change(function(){
        var search_id = $(this).val();
        $.ajax({
           url:"selectRegisteredStudents.php",  
           method:"POST",  
           ...
           {  
                $.ajax({  
                     url:"deleteRegisteredStudent.php",  
                     method:"POST",  
                     data:{id:id},  
                     dataType:"text",  
                     success:function(data){  
